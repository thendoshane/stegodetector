...
FEATURE_ORDER = [
    "file_size_bytes", "width", "height", "byte_entropy",
...
def upload_bytes(path: str, data: bytes, content_type: str = "image/png"):
...
def extract_features(img_bytes: bytes) -> Dict[str, float]:
    ...
    counts = np.bincount(np.frombuffer(img_bytes, dtype=np.uint8), minlength=256)
    probs = counts[counts > 0] / len(img_bytes)
    ...
    return {
        ...
        "r_var": float(np.var(arr[:, :, 0])),
        "g_mean": float(np.mean(arr[:, :, 1])),
        "g_var": float(np.var(arr[:, :, 1])),
        "b_mean": float(np.mean(arr[:, :, 2])),
        "b_var": float(np.var(arr[:, :, 2])),
    }
...
def _signed_url(storage_path: str, days: int = 365) -> str:
...
def _train_iforest_with_calibrated_threshold(
    scan_id: str,
    train_type: str,
    val_quantile: float = DEFAULT_VAL_QUANTILE,
    contamination: float = DEFAULT_CONTAMINATION
) -> Tuple[IsolationForest, StandardScaler, float, Dict[str, Any]]:
    docs = [d for d in _get_images(scan_id, train_type)]
    if len(docs) < 4:
        raise HTTPException(400, f"Need at least 4 '{train_type}' images (train+val).")

    rng = random.Random(RNG_SEED)
    ids = [d.id for d in docs]
    rng.shuffle(ids)
    split = int(len(ids) * 0.8)
    train_ids, val_ids = ids[:split], ids[split:] or ids[:1]

    def feats_for(image_id: str) -> List[float]:
        img = db.collection("scans").document(scan_id).collection("images").document(image_id).get().to_dict()
        if not img:
            raise HTTPException(404, f"Missing image doc: {image_id}")
        b = download_bytes(img["storagePath"])
        feats = extract_features(b)
        return [feats[k] for k in FEATURE_ORDER]

    X_train = np.array([feats_for(i) for i in train_ids], dtype=float)
    X_val = np.array([feats_for(i) for i in val_ids], dtype=float)
    ...
    info = {
        "trained_on": train_type,
        "train_size": int(len(train_ids)),
        "val_size": int(len(val_ids)),
        "contamination": float(contamination),
        "val_quantile": float(val_quantile),
        "threshold": float(threshold),
        "feature_order": FEATURE_ORDER,
        "rng_seed": RNG_SEED,
        "createdAt": firestore.SERVER_TIMESTAMP,
    }
    return model, scaler, threshold, info
...
def _embed_lsb(img: Image.Image, payload: bytes) -> Image.Image:
    ...
    # 3. Vectorized Embedding (Modifies array in-place, zero memory spike)
    # Clear LSB (bitwise AND with 254) and set new bit (bitwise OR)
...
def _extract_lsb_payload(img_bytes: bytes, max_payload_bytes: int = MAX_PAYLOAD_BYTES) -> bytes:
    """Extract payload embedded by _embed_lsb().

    Format: 4-byte big-endian length prefix + payload bytes,
    embedded bit-by-bit into the LSB of the RGB channel stream.
    """
    try:
    ...
    # First 32 bits = payload length (bytes), big-endian
    length_bits = lsb[:32]
    length = 0
    ...
    # Safety: packbits can pad, but we control length*8 so this is usually exact
...
def yara_scan(req: YaraReq, authorization: Optional[str] = Header(None)):
    ...
    for target in targets:
        ...
        for doc_snap in imgs:
            ...
            try:
                ...
                print(f"Skipping missing file: {img.get('filename')} (ID: {doc_snap.id})")
                continue 
            except Exception as e:
                print(f"Error downloading {img.get('filename')}: {str(e)}")
                continue
    ...
    return {"ok": True, "total": total, "detected": detected, "targets": per_target}
...
def anomaly(req: AnomReq, authorization: Optional[str] = Header(None)):
    ...
    model, scaler, threshold, info = _train_iforest_with_calibrated_threshold(
        scan_id=req.scanId,
        train_type=req.train_on,
        val_quantile=DEFAULT_VAL_QUANTILE,
        contamination=DEFAULT_CONTAMINATION,
    )

    os.makedirs("models", exist_ok=True)
    model_path = f"models/{req.scanId}_{req.train_on}_iforest.pkl"
    scaler_path = f"models/{req.scanId}_{req.train_on}_scaler.pkl"
    ...
    for mid in test_ids:
        ...
        try:
            b = download_bytes(img["storagePath"])
            feats = extract_features(b)
            vec = np.array([[feats[k] for k in FEATURE_ORDER]], dtype=float)

            raw = model.decision_function(scaler.transform(vec))[0]
            score = -float(raw)
            is_det = 1 if score >= threshold else 0
...
def hybrid(req: HybridReq, authorization: Optional[str] = Header(None)):
    ...
    return {"ok": True, "evaluated": total, "hybrid_detected": detected, "fusion": fusion}
...
def metrics(req: MetricsReq, authorization: Optional[str] = Header(None)):
        ...
        if missing_yara or missing_ml or missing_hyb:
            raise HTTPException(
                400,
                {
                    ...
                    "missing": {
                        "yara": missing_yara[:50],
                        "ml": missing_ml[:50],
                        "hybrid": missing_hyb[:50],
                    }
                }
            )
    ...
    def calc(y_true: List[int], y_pred: List[int]) -> Tuple[Dict[str, int], Dict[str, float]]:
        ...
        m = _metrics_from_counts(c) if y_true else {
            "accuracy": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0, "fpr": 0.0, "fnr": 0.0
        }
        return c, m
    ...
    out = {
        ...
        "misclassifications": {
            "hybrid_false_positives": fp,
            "hybrid_false_negatives": fn
        },
        "fusion": req.fusion.upper(),
    }
