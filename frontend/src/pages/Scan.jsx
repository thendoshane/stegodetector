...
export default function Scan() {
  ...
  // ML + fusion controls
  const [modelMode, setModelMode] = useState("workspace"); // "workspace" | "baseline"
  ...
  // baseline train controls
  const [baselineMaxTrain, setBaselineMaxTrain] = useState(600);
  const [baselineMaxVal, setBaselineMaxVal] = useState(200);
  ...
  useEffect(() => {
    ...
    const unsubImages = onSnapshot(
      query(collection(db, "scans", scanId, "images"), orderBy("createdAt", "desc")),
      (snap) => {
        const r = [];
    ...
    const unsubYara = onSnapshot(collection(db, "scans", scanId, "yara_results"), (snap) => {
      const r = [];
      snap.forEach((d) => r.push({ id: d.id, ...d.data() }));
      setYaraRows(r);
    });

    const unsubAnom = onSnapshot(collection(db, "scans", scanId, "anomaly_results"), (snap) => {
      const r = [];
      snap.forEach((d) => r.push({ id: d.id, ...d.data() }));
      setAnomRows(r);
    });

    const unsubHybrid = onSnapshot(collection(db, "scans", scanId, "hybrid_results"), (snap) => {
      const r = [];
      snap.forEach((d) => r.push({ id: d.id, ...d.data() }));
      setHybridRows(r);
    });

    const unsubVT = onSnapshot(collection(db, "scans", scanId, "virustotal_results"), (snap) => {
      const r = [];
      snap.forEach((d) => r.push({ id: d.id, ...d.data() }));
      setVtRows(r);
    });

    const unsubMetrics = onSnapshot(doc(db, "scans", scanId, "metrics", "latest"), (snap) => {
  ...
  const payloadPreview = useMemo(() => {
    ...
    if (payloadEncoding === "base64") {
      ...
      const approxBytes = Math.max(0, Math.floor((cleaned.length * 3) / 4) - pad);
      return { charCount: text.length, byteCount: approxBytes, note: "base64 estimate (backend validates)" };
    }
  ...
  async function apiCallFormData(endpoint, formData, method = "POST") {
    ...
    let token = await getToken();
    const opts = { method, headers: { Authorization: `Bearer ${token}` }, body: formData };
  ...
  async function deleteImage(img) {
    ...
    setBusy({ active: true, label: `Deleting ${img.filename || "image"}...` });
    try {
      await apiCall(`/image/${scanId}/${img.id}`, null, "DELETE");
    } catch (e) {
      alert(e?.message || String(e));
    } finally {
      setBusy({ active: false, label: "" });
  ...
  async function trainBaseline() {
    setBusy({ active: true, label: "Training baseline model (all workspaces originals)..." });
    try {
      await apiCall(`/models/baseline/train`, {
        scanId,
        max_train: Number(baselineMaxTrain),
        max_val: Number(baselineMaxVal),
      });
      alert("Baseline trained/updated.");
    } catch (e) {
      alert(e?.message || String(e));
    } finally {
      setBusy({ active: false, label: "" });
  ...
  function exportDecisionCSV() {
    downloadCSV(buildDecisionRows(), `decision_metrics_${scanId}.csv`);
  }
  ...
  function exportVTSummaryCSV() {
    const rows = (vtRows || []).map((r) => {
      ...
      const ratio = typeof det === "number" && typeof eng === "number" && eng ? (det / eng) : "";
      return {
        id: r.id,
        file: r.file || "",
        status: r.status || "",
    ...
    downloadCSV(rows, `vt_summary_${scanId}.csv`);
  }

  function exportVTEngineHitsCSV() {
    // One row per (file, engine) where engine is malicious/suspicious
    const rows = [];
    for (const r of (vtRows || [])) {
      const results = r.results || {};
      for (const [engine, category] of Object.entries(results)) {
    ...
    if (rows.length === 0) {
      alert("No malicious/suspicious engine hits to export (or VT not completed yet).");
      return;
    }
    downloadCSV(rows, `vt_engine_hits_${scanId}.csv`);
  }
  ...
  const yaraDetected = useMemo(() => yaraRows.filter((x) => Number(x.yara_detected) === 1), [yaraRows]);
  const yaraMissed = useMemo(() => yaraRows.filter((x) => Number(x.yara_detected) === 0), [yaraRows]);

  const anomDetected = useMemo(() => anomRows.filter((x) => Number(x.ml_detected) === 1), [anomRows]);
  const anomClean = useMemo(() => anomRows.filter((x) => Number(x.ml_detected) === 0), [anomRows]);

  const hybridDetected = useMemo(() => hybridRows.filter((x) => Number(x.final_pred) === 1), [hybridRows]);
  const hybridClean = useMemo(() => hybridRows.filter((x) => Number(x.final_pred) === 0), [hybridRows]);
  ...
  const Gallery = ({ title, list, allowDelete = true }) => (
    <div style={{ marginTop: 14 }}>
      ...
      {list.length === 0 ? (
        ...
        >
          {list.map((img) => (
            <div key={img.id} className="card" style={{ padding: 10, position: "relative" }}>
              {allowDelete && (
                <button
      ...
      <div style={{ padding: 24 }}>
        ...
        {activeTab === "upload" && (
          <div style={{ marginTop: 18 }}>
            ...
            <Gallery title="Original images (preview)" list={originals} allowDelete={true} />
            <Gallery title="Stego images (preview)" list={stegos} allowDelete={true} />
          </div>
        )}
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                Model mode:
                <select value={modelMode} onChange={(e) => setModelMode(e.target.value)} disabled={busy.active || !user}>
                  <option value="workspace">workspace (train this scan)</option>
                  <option value="baseline">baseline (all workspaces)</option>
                </select>
              </label>
                ...
                <>
                  <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      min={50}
                      max={5000}
                      value={baselineMaxTrain}
                      onChange={(e) => setBaselineMaxTrain(Number(e.target.value))}
                      style={{ width: 100 }}
                      disabled={busy.active || !user}
                    />
                  </label>
                  <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      min={10}
                      max={2000}
                      value={baselineMaxVal}
                      onChange={(e) => setBaselineMaxVal(Number(e.target.value))}
                      style={{ width: 90 }}
                      disabled={busy.active || !user}
                    />
                  </label>
                  ...
                  <button
                    ...
                    title="Trains/updates baseline using originals across ALL your workspaces"
                  >
              ...
              <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <select
                  ...
                  disabled={busy.active || !user || modelMode === "baseline"}
                  title={modelMode === "baseline" ? "Baseline ignores this (always clean originals across workspaces)" : ""}
                >
              ...
              <button
                ...
                disabled={!canRunAnom}
                onClick={() =>
                  run("/anomaly", "Running anomaly detection…", {
            ...
            <div style={{ marginTop: 10, fontSize: 12, opacity: 0.8 }}>
              Tip: If you want the ML to “remember” across sessions, use <b>baseline</b>. Workspace retrains each time by design.
            </div>
                    ...
                    {anomRows
                      ...
                      .map((r) => (
                        <tr key={r.id}>
                          ...
                          <td style={{ fontSize: 12 }}>{r.model_mode || "—"}</td>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <button
                ...
                title="Hybrid evaluates ONLY the intersection of IDs that have BOTH YARA + ML results."
              >
                Run Hybrid
              </button>
            ...
            <div style={{ marginTop: 12, opacity: 0.8, fontSize: 12 }}>
              Note: Hybrid rows are written only for the <b>intersection</b> (images that have BOTH YARA + ML results).
            </div>
        ...
        {activeTab === "vt" && (
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <button
                className="btn-secondary"
                disabled={busy.active || !user || vtRows.length === 0}
                onClick={exportVTSummaryCSV}
              >
                Export VT Summary CSV
              </button>

              <button
                className="btn-secondary"
                disabled={busy.active || !user || vtRows.length === 0}
                onClick={exportVTEngineHitsCSV}
                title="Exports only engines that marked malicious/suspicious"
              >
                Export VT Engine Hits CSV
              </button>
            ...
            {vtRows.length > 0 && (
              <div style={{ marginTop: 14 }}>
                ...
                <div style={{ marginTop: 10, fontSize: 12, opacity: 0.8 }}>
                  “Engine Hits CSV” is what you use in your thesis write-up for: <b>which AV engines flagged</b> (malicious/suspicious).
                </div>
              </div>
            )}
